...
ttesting <- datos[muestra,]
taprendizaje <- datos[-muestra,]
# Genera el modelo solo con las variables categóricas
modelo<-naiveBayes(BuenPagador~CoefCreditoAvaluo+GradoAcademico+IngresoNeto+MontoCuota,data=taprendizaje)
prediccion<-predict(modelo, ttesting[,1:5])
MC<-table(ttesting[,6],prediccion)
# Índices de Calidad de la predicción
indices.general(MC)
